...
type Message struct {
	...
	// ListUnsubscribe is the URL that ends a subscription, set only on mail a
	// person signed up for — the digest. Gmail and Outlook expect it on bulk
	// mail and treat its absence as a spam signal; putting it on a password
	// reset has the opposite effect, marking a one-off security message as
	// part of a mailing list.
	ListUnsubscribe string
	// ListUnsubscribeOneClick promises that a bare POST to ListUnsubscribe ends
	// the subscription with no further interaction. Only set it when an
	// endpoint actually honours that: a client may POST without ever showing
	// the reader a page, and a broken promise silently strands them subscribed.
	ListUnsubscribeOneClick bool
	// AutoGenerated marks the message as machine-sent. It suppresses
	// out-of-office replies bouncing back at a no-reply address, and tells a
	// filter this is service mail rather than something a person composed.
	AutoGenerated bool
}
