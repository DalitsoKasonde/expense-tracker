...
type outgoing struct {
	...
	// ListUnsubscribe is set only on mail a person subscribed to, and carries
	// the page that turns it off. Transactional mail leaves it empty: a
	// password reset is not a subscription, and advertising one as such marks
	// a security message as bulk.
...
func (m *mailer) send(ctx context.Context, message outgoing) error {
	...
	sendErr := m.sender.Send(ctx, mail.Message{
		...
		// Everything this app sends is generated by the app, never typed by a
		// person, so no reply-bot should answer it.
		AutoGenerated:   true,
