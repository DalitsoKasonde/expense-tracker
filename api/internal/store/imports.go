...
func (s *ImportStore) Create(ctx context.Context, userID, fileName string) (Import, error) {
	...
	err := s.db.QueryRow(ctx, `
		...
		values ($1, $2, '', 'uploaded')
		returning id, user_id, file_name, status, created_at::text, updated_at::text
	`, userID, fileName).Scan(
		&imp.ID, &imp.UserID, &imp.FileName, &imp.Status, &imp.CreatedAt, &imp.UpdatedAt,
	)
	return imp, err
}
...
func (s *ImportStore) GetByID(ctx context.Context, id, userID string) (Import, error) {
	var imp Import
	err := s.db.QueryRow(ctx, `
		select id, user_id, file_name, status, error_message, created_at::text, updated_at::text
		from imports
		where id = $1 and user_id = $2
	`, id, userID).Scan(
		&imp.ID, &imp.UserID, &imp.FileName, &imp.Status, &imp.Error, &imp.CreatedAt, &imp.UpdatedAt,
	)
