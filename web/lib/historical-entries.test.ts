...
describe("supportsHistoricalBackfill", () => {
  it("allows expenses, savings, lending advances, and historical investment activity", () => {
    ...
    expect(supportsHistoricalBackfill("investment_income")).toBe(true);
    expect(supportsHistoricalBackfill("dividend_drip")).toBe(true);
    expect(supportsHistoricalBackfill("loan_receivable_advance")).toBe(true);
  });
  ...
  it("refuses kinds that need the account on the other side of the entry", () => {
    ...
    expect(supportsHistoricalBackfill("loan_receivable_repayment")).toBe(false);
